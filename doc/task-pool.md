...
Resuming a paused requirement whose execution exhausted retries releases the pause into `needs_attention`, preserving attempts and retry limits. It does not automatically grant another attempt.

### Transient transport retries

`maxTransportRetries` defaults to 2 (0–5), independently of `maxAttempts`. A confirmed failed adapter run with connection/reset/DNS/timeout transport errors gets this separate budget; authentication failures and worker validation failures do not. Attempts record `failureKind` and retain their individual worktrees. Persisted exponential backoff applies, and all automatic attempts stop at a total of 10. Existing requirements default to two transport retries for future failures; historical blocked work is not silently reopened. Use a diagnosed `retry_task` intervention to recover existing blocked work.
