...
Connection failures have a separate `maxTransportRetries` create option (default 2, range 0–5). This permits recovery even when coding `maxAttempts` is 1. Only confirmed adapter transport failures qualify; authentication and validation failures do not. `attempt.failureKind` distinguishes transport and execution failures. Old blocked requirements are not automatically reopened by an upgrade: inspect the failure and use `retry_task` with corrective feedback. Previous worktrees remain read-only evidence for the new worker.
